...
## 8. 实施状态对账（2026-09-12 更新）

本文档撰写时 Flow 尚未实现；截至本次更新，Flow 已基本落地并完成一轮系统性修复与增强。本节记录设计与实现的偏差，**阅读前文时应以本节为准**。

### 8.1 已落地能力（超出原设计范围）

| 能力 | 状态 | 说明 |
| --- | --- | --- |
| 条件分支 | ✅ 已实现（§2.7 原计划首版不做） | `FlowEdge.condition` 支持无条件/简单条件/JS 表达式三种模式，边条件在节点执行后求值选择分支 |
| 节点级断言感知 | ✅ 已实现 | 断言失败判节点 failed，走错误策略与条件分支；总览展示逐条断言明细；单跑节点同样生效但不触发策略 |
| 运行历史持久化 | ✅ 已实现 | `userData/flow-runs/` 按 collectionUid/flowUid 组织，滚动保留 20 次，响应体超 1MB 截断；工作台可回看历史运行的任意节点响应 |
| 画布脏标记与未保存保护 | ✅ 已实现 | flowDirtyMiddleware + dirty-registry；顶栏脏点、关闭 Tab 拦截确认、树重建/外部 flow.yml 变更不覆盖未保存图 |
| 撤销/重做、右键菜单、自动布局 | ✅ 已实现 | 原设计未覆盖 |
| 三级运行（全局/单跑节点/运行到此） | ✅ 已实现 | 原设计仅全局运行 |

### 8.2 与设计约束的对齐修正（2026-09-12 修复批次）

原 §2.7 与 §6 禁令中列出的两条违规已修复：

1. **取消**（原违规："伪取消"）：`cancelTokenUid` 现已从执行器贯通 `sendNetworkRequest` → IPC `send-http-request` → 主进程 `runRequest` 的 AbortController；取消响应按 `isCancel` 识别，当前节点标 `cancelled`、下游 `skipped`、整体标 `cancelled`。
2. **变量泄漏**（原违规："上游 body 自动泄漏"）：`runtimeVariables` 每节点独立构建（集合基础变量 + 本节点映射输入），跨节点不再残留。

### 8.3 实现语义的补充决定

- **continue 后的出边条件**：failed+continue 的节点按"已执行、失败"参与分支选择（`flowContext.status` 记 0 表示无 HTTP 响应），不再简单线性走下一个。
- **分支终止语义**：所有出边条件不满足且无默认边时，流程标 `failed` 并在总览给出原因，**不再误报成功**。
- **jump 策略**：真跳转到目标节点（途中节点标 `skipped`）；目标不可达/回跳时报错终止。
- **回边/循环**：仍不支持（图校验判环报错），循环迭代（ForEach）与并行分支为未来方向。
- **表达式根字段**：`$flow.<stepId>` 路径支持 `body/status/statusText/headers/duration`（原设计仅 body）；条件求值使用受限求值器（`safe-eval`），不再使用裸 `new Function`。
- **单跑节点**：不适用错误策略（链路概念），断言失败同样标 failed。
- **运行历史**：单跑节点不产生历史记录。

### 8.4 测试基线

- `packages/bruno-app`：151 套件 / 2181 用例全绿（含 executor/条件/策略/取消/断言/脏标记/运行历史等 Flow 专项用例）。
- 交接文档 §4 测试矩阵中"执行器并发拒绝、取消信号、映射失败终止"均已补齐单测。
